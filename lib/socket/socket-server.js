...
export function initSocketServer(server) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('join-session', (sessionId) => {
      ...
      logger.info(`[Socket.IO] Client ${socket.id} joined session: ${sessionId}`);
    ...
    socket.on('leave-session', (sessionId) => {
      ...
      logger.info(`[Socket.IO] Client ${socket.id} left session: ${sessionId}`);
    ...
    socket.on('chat-message', async (data) => {
      ...
      logger.info(`[Socket.IO] Chat message received in session ${sessionId} from ${senderId}`);
