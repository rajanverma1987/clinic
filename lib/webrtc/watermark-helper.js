...
export function applyWatermarkToStream(stream, userId) {
  ...
  if (!videoTrack) {
    logger.warn('[Watermark] No video track found in stream');
    return stream;
  }
...
export async function applyWatermarkWithInsertableStreams(stream, userId) {
  if (!stream.getVideoTracks()[0]?.getSettings) {
    logger.warn('[Watermark] Insertable streams not supported, falling back to canvas');
  ...
  try {
    ...
    logger.warn('[Watermark] Insertable streams failed, using canvas fallback:', error);
    return applyWatermarkToStream(stream, userId);
  }
}
