        ...
        switch (message.type) {
          case 'offer':
            ...
            try {
              ...
              const localTracksAfterSetRemote = this.peerConnection.localStream.getTracks();
              console.log('[VideoCallManager] After setRemoteDescription - Senders:', {
                count: sendersAfterRemoteDesc.length,
                localTracks: localTracksAfterSetRemote.length,
              ...
              // Ensure all local tracks are still in senders after setRemoteDescription
              localTracksAfterSetRemote.forEach(track => {
                const hasTrack = sendersAfterRemoteDesc.some(sender => sender.track === track);
                if (!hasTrack && track.readyState === 'live') {
                  console.log('[VideoCallManager] ➕ Re-adding local track after setRemoteDescription:', track.kind);
                  this.peerConnection.peerConnection.addTrack(track, this.peerConnection.localStream);
                }
              });
