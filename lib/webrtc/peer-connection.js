...
export class WebRTCPeerConnection {
  ...
  initializePeerConnection() {
    ...
    this.peerConnection.ontrack = (event) => {
      ...
      if (event.track) {
        ...
        event.track.onmute = () => {
          console.warn('[PeerConnection] Remote track muted:', event.track.kind);
        };

        event.track.onunmute = () => {
          console.log('[PeerConnection] Remote track unmuted:', event.track.kind);
        };
      }

      // Notify callback whenever a track is added
  ...
  async createAnswer() {
    ...
    if (this.localStream) {
      ...
      const existingTracks = existingSenders.map(sender => sender.track).filter(Boolean);
      const localTracks = this.localStream.getTracks();

      console.log('[PeerConnection] Pre-answer track check:', {
        existingSenders: existingSenders.length,
        existingTracks: existingTracks.length,
        localTracks: localTracks.length,
        localTrackKinds: localTracks.map(t => t.kind),
        localTrackStates: localTracks.map(t => t.readyState)
      });

      // Add any missing tracks
      localTracks.forEach(track => {
        if (track && track.readyState === 'live') {
          const hasTrack = existingTracks.includes(track);
          if (!hasTrack) {
            console.log('[PeerConnection] ➕ Adding local track to peer connection before creating answer:', {
              kind: track.kind,
              id: track.id,
            ...
            try {
              ...
              console.error('[PeerConnection] ❌ Failed to add track:', error);
            }
          } else {
            console.log('[PeerConnection] ✓ Local track already in senders:', track.kind);
          }
        } else {
          console.warn('[PeerConnection] ⚠️ Local track not ready:', {
            kind: track?.kind,
            readyState: track?.readyState
          });
        }
      ...
      // Final verification
      const finalSenders = this.peerConnection.getSenders();
      console.log('[PeerConnection] Post-add check before answer:', {
        senders: finalSenders.length,
        senderTracks: finalSenders.map(s => ({
          kind: s.track?.kind,
          id: s.track?.id,
      ...
      console.error('[PeerConnection] ❌ No local stream available when creating answer!');
    ...
    if (answer.sdp) {
      ...
      console.log('[PeerConnection] Answer SDP verification:', { hasAudio, hasVideo });
    }
