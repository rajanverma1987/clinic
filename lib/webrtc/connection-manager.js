...
export class ConnectionManager {
  ...
  checkConnectionState(peerConnection) {
    ...
    if (this.connectionState !== status) {
      this.updateConnectionState(status, `ICE: ${iceState}, Connection: ${connectionState}`);
    }
  ...
  attemptReconnect() {
    ...
    if (this.reconnectAttempts >= this.maxReconnectAttempts) {
      ...
      this.updateConnectionState('failed', 'Max reconnection attempts reached');
      this.onError(new Error('Connection failed after maximum reconnection attempts'));
      return;
    }
