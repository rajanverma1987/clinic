  ...
  async getUserMedia() {
    try {
      ...
      if (videoTrack && videoTrack.getCapabilities) {
        ...
        console.log('[WebRTC] Video capabilities:', {
        ...
        if (videoTrack.applyConstraints) {
          try {
            ...
            console.log('[WebRTC] ✅ Applied high-quality video constraints');
          } catch (constraintError) {
            console.warn('[WebRTC] Could not apply ideal constraints, using defaults:', constraintError);
          }
        }
      }
  ...
  async initialize() {
    ...
    const sdpTransform = (sdp) => {
      try {
        ...
        console.log('[WebRTC] ✅ SDP transformed for high quality');
        return sdp;
      } catch (error) {
        console.warn('[WebRTC] ⚠️ SDP transform error, using original SDP:', error);
    ...
    console.log('[WebRTC] Peer created with trickle ICE enabled (better for TURN servers)');
    ...
    if (this.isInitiator) {
      console.log('[WebRTC] 🎯 This peer is the INITIATOR - will generate SDP offer');
    } else {
      console.log('[WebRTC] 🎯 This peer is the RECEIVER - waiting for SDP offer from initiator');
    }
    ...
    this.peer.on('stream', (stream) => {
      ...
      if (remoteVideoTrack) {
        ...
        console.log('[WebRTC] Remote video quality:', {
    ...
    this.peer.on('connect', () => {
      ...
      if (this.onConnect) {
        try {
          ...
          console.error('[WebRTC] Error in onConnect callback:', error);
        }
      }
    });
    ...
    this.peer.on('close', () => {
      ...
      if (this.onClose) {
        try {
          ...
          console.error('[WebRTC] Error in onClose callback:', error);
        }
      }
    });
    ...
    this.peer.on('error', (error) => {
      ...
      if (this.onError) {
        try {
          ...
          console.error('[WebRTC] Error in onError callback:', err);
        }
      }
    });
