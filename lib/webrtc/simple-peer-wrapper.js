...
export class WebRTCPeerWrapper {
  ...
  async initialize() {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (turnServer) {
        console.log('[WebRTC] Adding TURN server for proxy/NAT traversal:', turnServer);
        const turnConfig = {
          urls: turnServer
        };
        ...
        console.warn('[WebRTC] No TURN server configured. Connection may fail if both peers are behind NAT/firewalls.');
        console.warn('[WebRTC] For proxy scenarios, set NEXT_PUBLIC_TURN_SERVER_URL in .env.local');
        console.warn('[WebRTC] See CursorMD/WEBRTC_PROXY_TURN_SETUP.md for setup instructions');
      }
    }
  ...
  signal(data) {
    if (this.peer) {
      ...
      console.warn('[WebRTC] Cannot process signal - peer not initialized');
    }
  }
