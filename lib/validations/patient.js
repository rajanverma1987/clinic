...
export const patientRegistrationSchema = z.object({
  ...
  alternatePhone: z.string().max(20, 'Alternate phone number is too long').optional().or(z.literal('')),
  address: z.object({
...
export const patientSearchSchema = z.object({
  ...
  page: z.string().or(z.number()).optional().transform((val) => {
    const num = typeof val === 'string' ? parseInt(val, 10) : val;
    return isNaN(num) || num < 1 ? 1 : num;
  }),
  limit: z.string().or(z.number()).optional().transform((val) => {
    const num = typeof val === 'string' ? parseInt(val, 10) : val;
    return isNaN(num) || num < 1 || num > 100 ? 20 : num;
  }),
  sortBy: z.enum(['firstName', 'lastName', 'dateOfBirth', 'createdAt', 'patientId']).optional(),
