...
import { PRIMARY_100, PRIMARY_900 } from '@/lib/constants/brand-colors';
...
async function getTransporter(tenantId = null) {
  ...
  if (tenantId) {
    try {
      ...
      logger.warn('Failed to load tenant SMTP settings, using defaults:', error.message);
    }
  }
  ...
  if (!smtpConfig.host) {
    logger.warn(
      '⚠️ SMTP not configured. Emails will not be sent. Set SMTP_HOST, SMTP_PORT, SMTP_USER, and SMTP_PASSWORD environment variables.',
    );
    return null;
  }
...
async function getFromEmail(tenantId = null) {
  if (tenantId) {
    try {
      ...
      logger.warn('Failed to load tenant from email, using defaults:', error.message);
    }
  }
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <div style="background: #f9fafb; padding: 30px; border-radius: 0 0 10px 10px;">
    ...
    <div style="background: ${PRIMARY_100}; border-left: 4px solid ${PRIMARY_900}; padding: 15px; margin: 25px 0; border-radius: 4px;">
...
/**
 * Send doctor verification result email (approve/reject)
 * Used when admin approves or rejects a doctor application (dp-6)
 */
export async function sendDoctorVerificationResultEmail(
  doctorEmail,
  doctorName,
  action,
  comment = '',
  ...
  const subject = isApproved
    ...
    : 'Update on Your Doctor Verification Application';
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>${isApproved ? 'Verification Approved' : 'Verification Update'}</title>
</head>
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  <div style="background: linear-gradient(135deg, ${isApproved ? '#10b981 0%, #059669 100%' : '#6b7280 0%, #4b5563 100%'}); padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
    <h1 style="color: white; margin: 0; font-size: 28px;">${isApproved ? '✓ Account Verified' : 'Verification Update'}</h1>
  </div>
  <div style="background: #f9fafb; padding: 30px; border-radius: 0 0 10px 10px;">
    <p style="font-size: 16px; margin-bottom: 20px;">Dear ${doctorName},</p>
    ${
      isApproved
        ? `<p style="font-size: 16px; margin-bottom: 20px;">Your doctor profile has been <strong>verified</strong>. You can now log in and start using the platform.</p>
    <div style="background: #d1fae5; border-left: 4px solid #10b981; padding: 15px; margin: 25px 0; border-radius: 4px;">
      <p style="margin: 0; font-size: 14px; color: #065f46;">You're all set. Log in to manage your schedule, see patients, and access your dashboard.</p>
    </div>`
        : `<p style="font-size: 16px; margin-bottom: 20px;">After review, your doctor verification application was <strong>not approved</strong> at this time.</p>
    ${
      comment
        ? `<div style="background: #fee2e2; border-left: 4px solid #ef4444; padding: 15px; margin: 25px 0; border-radius: 4px;">
      <p style="margin: 0 0 8px 0; font-size: 14px; color: #991b1b;"><strong>Reason / comments:</strong></p>
      <p style="margin: 0; font-size: 14px; color: #991b1b;">${comment}</p>
    </div>`
        : ''
    }
    <p style="font-size: 14px; margin-bottom: 20px;">You may reapply or submit additional documents if required. If you have questions, please contact support.</p>`
    }
    <p style="font-size: 14px; color: #6b7280; margin-top: 30px;">
      Best regards,<br>
      <strong>Doctor's Clinic Team</strong>
  ...
  const text = isApproved
    ? `Dear ${doctorName},\n\nYour doctor profile has been verified. You can now log in and start using the platform.\n\nBest regards,\nDoctor's Clinic Team`
    : `Dear ${doctorName},\n\nAfter review, your doctor verification application was not approved at this time.${comment ? `\n\nReason / comments: ${comment}` : ''}\n\nYou may reapply or submit additional documents. If you have questions, please contact support.\n\nBest regards,\nDoctor's Clinic Team`;
...
/**
 * Send doctor document request email (admin requests additional documents)
 */
export async function sendDoctorDocumentRequestEmail(
  doctorEmail,
  doctorName,
  documentType,
  comment = '',
  ...
  const subject = 'Additional Documents Required for Verification';
  const html = `
<!DOCTYPE html>
<html>
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  <div style="background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%); padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
    <h1 style="color: white; margin: 0; font-size: 28px;">Additional Documents Required</h1>
  </div>
  <div style="background: #f9fafb; padding: 30px; border-radius: 0 0 10px 10px;">
    <p style="font-size: 16px;">Dear ${doctorName},</p>
    <p style="font-size: 16px;">To complete your verification, we need the following:</p>
    <p style="font-size: 16px;"><strong>${documentType}</strong></p>
    ${comment ? `<p style="font-size: 14px; color: #6b7280;">${comment}</p>` : ''}
    <p style="font-size: 14px; margin-top: 20px;">Please log in and upload the requested documents.</p>
    <p style="font-size: 14px; color: #6b7280; margin-top: 30px;">Best regards,<br><strong>Doctor's Clinic Team</strong></p>
  </div>
</body>
</html>
  `;
  const text = `Dear ${doctorName},\n\nTo complete your verification, we need: ${documentType}.${comment ? `\n\n${comment}` : ''}\n\nPlease log in and upload the requested documents.\n\nBest regards,\nDoctor's Clinic Team`;
  return await sendEmail({ to: doctorEmail, subject, html, text }, tenantId);
}
