...
async function getTransporter(tenantId = null) {
  ...
  if (!smtpConfig.host) {
    logger.warn('⚠️ SMTP not configured. Emails will not be sent. Set SMTP_HOST, SMTP_PORT, SMTP_USER, and SMTP_PASSWORD environment variables.');
    return null;
  }
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <div style="background: #f9fafb; padding: 30px; border-radius: 0 0 10px 10px;">
    ...
    ${comment ? `<div style="background: #fee2e2; border-left: 4px solid #ef4444; padding: 15px; margin: 25px 0; border-radius: 4px;">
