...
const OAUTH_PROVIDERS = {
  google: {
    ...
    redirectUri: process.env.GOOGLE_REDIRECT_URI || `${process.env.NEXT_PUBLIC_APP_URL}/api/auth/oauth/callback`,
  ...
  apple: {
    ...
    redirectUri: process.env.APPLE_REDIRECT_URI || `${process.env.NEXT_PUBLIC_APP_URL}/api/auth/oauth/callback`,
...
export async function handleOAuthCallback(provider, code, state) {
  ...
  if (!user) {
    ...
    if (existingUser) {
      ...
      // Create new user (patient portal only - clinic admins must register normally)
