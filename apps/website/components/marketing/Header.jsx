...
/**
 * @param {boolean} [noFixed] - When true, header is not position:fixed (for use inside a fixed wrapper e.g. with FreeTrialBanner).
 */
export function Header({ noFixed = false }) {
  ...
  return (
    <header
      className={`transition-smooth ${
        noFixed ? 'relative' : 'fixed top-0 left-0 right-0 z-[9999]'
      } ${
        scrolled
    ...
    >
      <nav className='max-w-7xl mx-auto section-padding-x'>
        <div className='flex justify-between items-center h-20'>
          {/* Logo Section */}
          <div className='flex items-center'>
            <Link href='/' className='flex items-center'>
              <div className='relative flex items-center justify-center w-[180px] h-[50px]'>
                <Image
          ...
          <div className='hidden lg:flex items-center gap-1'>
            {navigationLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className='text-body-md text-neutral-700 font-medium relative group overflow-hidden rounded-lg px-5 py-2.5 transition-smooth'
              >
                <span className='relative z-10 group-hover:text-primary-500 transition-smooth-fast'>
                  {link.label}
                </span>
                {/* Background hover effect */}
                <span className='absolute inset-0 bg-primary-50 scale-x-0 group-hover:scale-x-100 origin-left rounded-lg transition-smooth'></span>
                {/* Underline effect */}
                <span className='absolute bottom-0 left-0 h-0.5 w-0 bg-primary-500 group-hover:w-full rounded-full transition-smooth'></span>
              </Link>
            ))}
          </div>
                ...
                <div className='hidden md:flex bg-white rounded-xl border border-neutral-200 p-3 shadow-sm hover:shadow-md'>
                  <div className='flex items-center gap-3'>
                    ...
                    <div className='relative flex-shrink-0'>
                      <div className='bg-gradient-to-br from-primary-500 to-primary-700 rounded-lg flex items-center justify-center text-white font-semibold w-[var(--size-2xl)] h-[var(--size-2xl)] text-body-md'>
                        {user?.firstName?.[0]?.toUpperCase() || 'U'}
                        {user?.lastName?.[0]?.toUpperCase() || ''}
                      </div>
                      {/* Status Dot */}
                      <div
                        className={`absolute -bottom-0.5 -right-0.5 border-2 border-white rounded-full shadow-md w-[var(--space-3)] h-[var(--space-3)] ${
                    ...
                    <div className='flex-1 min-w-0'>
                      <p className='text-neutral-900 font-semibold truncate text-body-sm'>
                        {user.role === 'doctor'
                          ? `Dr. ${user.firstName} ${user.lastName}`
                          : `${user.firstName} ${user.lastName}`}
                      </p>
                      <p className='text-neutral-600 truncate text-body-xs'>
