...
export function PrescriptionPrintView({ prescriptionData }) {
  ...
  return (
    <div className='prescription-print p-8 max-w-4xl mx-auto bg-white'>
        ...
        <div className='flex justify-between items-start'>
          ...
          <div className='text-right'>
            ...
            <p className='text-sm text-gray-600'>
              Valid Until: {formatDate(prescriptionData.validUntil)}
            </p>
          </div>
        </div>
      </div>
      ...
      {/* Patient Information */}
      <div className='mb-6 content-grid-2 content-grid-gap-6'>
        <div>
          ...
          <div className='space-y-1 text-sm'>
            <p>
              <span className='font-medium'>Name:</span> {prescriptionData.patientId?.firstName}{' '}
              {prescriptionData.patientId?.lastName}
            </p>
            <p>
              <span className='font-medium'>Patient ID:</span>{' '}
              {prescriptionData.patientId?.patientId || 'N/A'}
            </p>
            {prescriptionData.patientId?.dateOfBirth && (
              <p>
                <span className='font-medium'>DOB:</span>{' '}
                {formatDate(prescriptionData.patientId.dateOfBirth)}
              </p>
            )}
            {prescriptionData.patientId?.phone && (
              <p>
                <span className='font-medium'>Phone:</span> {prescriptionData.patientId.phone}
              </p>
            )}
          </div>
        </div>
        <div>
          ...
          <div className='space-y-1 text-sm'>
            <p className='font-medium'>
              Dr. {prescriptionData.doctorId?.firstName} {prescriptionData.doctorId?.lastName}
            </p>
            ...
            {prescriptionData.items.map((item, index) => (
              <tr key={index} className='hover:bg-gray-50'>
                ...
                <td className='border border-gray-300 px-4 py-3 text-sm'>
                  {getItemTypeLabel(item.itemType || 'drug')}
                </td>
                <td className='border border-gray-300 px-4 py-3 text-sm'>
                  {item.itemType === 'drug' && (
                    <div>
