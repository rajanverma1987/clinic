 ...
 * Based on NEW-PLANS.md requirements; uses AUDIT_CONFIG from route-security for retention and always-audit routes.
 */

import { AUDIT_CONFIG, getDataClassification } from '@/lib/constants/route-security';
...
export async function createAuditLog({
  ...
  try {
    ...
    const pathHint =
      resource === 'patient'
        ? `/patients${validResourceId ? `/${validResourceId}` : ''}`
        : resource === 'invoice'
          ? `/invoices${validResourceId ? `/${validResourceId}` : ''}`
          : null;
    const dataClassification = pathHint ? getDataClassification(pathHint) : null;
    const detailsWithClassification = dataClassification
      ? {
          ...details,
          _classification: {
            phiLevel: dataClassification.phiLevel,
            retentionYears: dataClassification.retentionYears,
          },
        }
      : details;
    ...
    const auditLog = new AuditLog({
      ...
      details: detailsWithClassification,
    ...
    const isMongoAuthFailure =
      error &&
      (error.code === 18 ||
        (typeof error.message === 'string' && error.message.includes('Authentication failed')));
    if (isMongoAuthFailure) {
      logger.warn(
        'MongoDB auth failed. Check MONGODB_URI in apps/clinic/.env.local (username, password, authSource=admin).',
      );
    }
...
/**
 * Audit Action Enum (aligned with AUDIT_CONFIG.auditActions where applicable)
 */
...
/**
 * Whether a path or resource should always be audited (from AUDIT_CONFIG.alwaysAudit).
 * @param {string} pathOrResource - e.g. '/patients/123', '/prescriptions', 'patient', 'invoice'
 */
export function shouldAlwaysAudit(pathOrResource) {
  if (!pathOrResource) return false;
  const s = String(pathOrResource).toLowerCase();
  return AUDIT_CONFIG.alwaysAudit.some((p) => {
    const pathPart = p.split('/').filter(Boolean)[0];
    if (pathPart && (s.includes(pathPart) || s === pathPart.replace(/s$/, ''))) return true;
    const pattern = '^' + p.replace(/:[^/]+/g, '[^/]+') + '$';
    return new RegExp(pattern).test(s);
  });
}

/** Default retention for audit logs (days); from AUDIT_CONFIG.retentionDays */
export const AUDIT_RETENTION_DAYS = AUDIT_CONFIG.retentionDays;
