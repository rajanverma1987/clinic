...
export class VideoCallManager {
  ...
  async startCall() {
    try {
      ...
      logger.info('[VideoCallManager] Initializing peer wrapper...');
      await this.peerWrapper.initialize();
      logger.info('[VideoCallManager] ✅ Peer wrapper initialized');

      // Get local stream and display it
      const localStream = this.peerWrapper.localStream;
      if (localStream && this.onLocalStream) {
        logger.info('[VideoCallManager] ✅ Local stream ready, displaying video');
        this.onLocalStream(localStream);
      } else {
        logger.warn('[VideoCallManager] ⚠️ Local stream not available');
      }

      // Start signaling polling
      logger.info('[VideoCallManager] Starting signaling poll...');
      ...
      logger.info('[VideoCallManager] ✅ Call started. Waiting for remote peer...');
      logger.info('[VideoCallManager] Call info:', {
        sessionId: this.sessionId,
        userId: this.userId,
  ...
  async handleSignal(data) {
    if (!this.apiClient) {
      logger.error('[VideoCallManager] apiClient not provided');
      return;
    }
    ...
    try {
      ...
      if (!response.success) {
        logger.error('[VideoCallManager] ❌ Failed to send signal:', response.error);
        logger.error('[VideoCallManager] Response:', response);
      } else {
        logger.info('[VideoCallManager] ✅ Signal sent successfully to', this.remoteUserId);
      ...
      logger.error('[VideoCallManager] Error sending signal:', error);
    }
  }
  ...
  startSignalingPoll() {
    ...
    this.signalingInterval = setInterval(async () => {
      ...
      try {
        ...
        logger.info('[VideoCallManager] Polling for signals:', pollUrl);

        const response = await this.apiClient.get(pollUrl);

        logger.info('[VideoCallManager] Poll response:', {
        ...
        if (response.success && response.data && response.data.signals) {
          ...
          for (const signal of signals) {
            logger.info('[VideoCallManager] Processing signal:', {
              id: signal.id,
              from: signal.from,
            ...
            if (this.peerWrapper) {
              try {
                ...
                logger.info('[VideoCallManager] ✅ Signal processed successfully');
              } catch (error) {
        ...
        logger.error('[VideoCallManager] Error polling for signals:', error);
      }
    ...
    const connectionCheckInterval = setInterval(() => {
      ...
      if (connectionState === 'failed' || connectionState === 'disconnected') {
        ...
        logger.warn('[VideoCallManager] ⚠️ Connection issue detected:', connectionState);
        ...
        logger.info('[VideoCallManager] ✅ ICE connection successful!');
    ...
    setTimeout(() => {
      ...
      if (!this.peerWrapper?.isConnected && this.isCallActive) {
        ...
        if (connectionState === 'checking' || connectionState === 'new') {
          logger.warn('[VideoCallManager] ⏳ Still connecting after timeout, but continuing...');
          // Don't fail - let ConnectionManager continue trying
        } else if (connectionState === 'failed') {
          // ConnectionManager will handle reconnection
          logger.warn('[VideoCallManager] ⚠️ Connection failed, reconnection will be attempted');
        }
      }
  ...
  handleRemoteStream(stream) {
    logger.info('[VideoCallManager] ✅ Remote stream received');
   ...
   */
  handleConnect() {
    logger.info('[VideoCallManager] ✅ Peer connection established');
   ...
   */
  handleError(error) {
    logger.error('[VideoCallManager] Error:', error);
    this.isCallActive = false;
    // Don't immediately fail - let ConnectionManager handle reconnection
    if (this.onError) {
      this.onError(error);
  ...
  handleConnectionStateChange(state) {
    logger.info('[VideoCallManager] Connection state change:', state);
  ...
  handleQualityChange(quality) {
    ...
    if (quality.quality && quality.quality !== 'UNKNOWN') {
      logger.info('[VideoCallManager] Connection quality:', quality.quality);
    }
  }
  ...
  async handleReconnect(info) {
    ...
    try {
      ...
      logger.info('[VideoCallManager] ✅ Reconnection successful');
    } catch (error) {
      logger.error('[VideoCallManager] ❌ Reconnection failed:', error);
  ...
  async startScreenShare() {
    try {
      ...
      if (this.peerWrapper && this.peerWrapper.peer && this.peerWrapper.peer._pc) {
        ...
        videoTrack.onended = () => {
          logger.info('[VideoCallManager] Screen share ended by user');
      ...
      logger.error('[VideoCallManager] Failed to start screen share:', error);
      throw error;
    }
  }
    ...
    if (this.screenShareStream) {
      ...
      if (
        ...
        if (videoSender && videoTrack) {
          await videoSender.replaceTrack(videoTrack);
          logger.info('[VideoCallManager] ✅ Camera track restored');
        } else {
          logger.warn('[VideoCallManager] ⚠️ No video sender found to restore camera');
        }
      }
    }
  }
   ...
   */
  async endCall() {
    logger.info('[VideoCallManager] Ending call...');
    ...
    logger.info('[VideoCallManager] ✅ Call ended');
  }
