...
export class CallRecorder {
  ...
  getSupportedMimeType() {
    ...
    for (const type of types) {
      if (MediaRecorder.isTypeSupported(type)) {
        logger.info('[CallRecorder] Using MIME type:', type);
        return type;
      }
    }

    // Fallback to default
    logger.warn('[CallRecorder] No specific MIME type supported, using default');
    return '';
  }
  ...
  async startRecording() {
    if (this.isRecording) {
      logger.warn('[CallRecorder] Already recording');
      return;
    }
    ...
    try {
      ...
      this.mediaRecorder.ondataavailable = (event) => {
        if (event.data && event.data.size > 0) {
          ...
          logger.info('[CallRecorder] Data chunk received:', event.data.size, 'bytes');
        }
      };

      // Handle recording stop
      this.mediaRecorder.onstop = () => {
        logger.info('[CallRecorder] Recording stopped');
        this.isRecording = false;
      ...
      this.mediaRecorder.onerror = (event) => {
        logger.error('[CallRecorder] Recording error:', event.error);
      ...
      logger.info('[CallRecorder] ✅ Recording started');
      this.onRecordingStateChange({
        isRecording: true,
        duration: 0
      });
      ...
      logger.error('[CallRecorder] Failed to start recording:', error);
  ...
  async stopRecording() {
    if (!this.isRecording || !this.mediaRecorder) {
      logger.warn('[CallRecorder] Not recording');
      return null;
    }
    ...
    return new Promise((resolve, reject) => {
      this.mediaRecorder.onstop = () => {
        try {
          ...
          logger.info('[CallRecorder] Recording stopped, blob size:', blob.size, 'bytes');
  ...
  pauseRecording() {
    if (this.mediaRecorder && this.mediaRecorder.state === 'recording') {
      ...
      logger.info('[CallRecorder] Recording paused');
    }
  }
  ...
  resumeRecording() {
    if (this.mediaRecorder && this.mediaRecorder.state === 'paused') {
      ...
      logger.info('[CallRecorder] Recording resumed');
    }
  }
