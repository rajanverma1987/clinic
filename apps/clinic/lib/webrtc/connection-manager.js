...
export class ConnectionManager {
  ...
  setupNetworkListeners() {
    window.addEventListener('online', () => {
      logger.info('[ConnectionManager] ✅ Network back online');
    ...
    window.addEventListener('offline', () => {
      logger.info('[ConnectionManager] ❌ Network offline');
  ...
  setupVisibilityListeners() {
    document.addEventListener('visibilitychange', () => {
      if (document.hidden) {
        logger.info('[ConnectionManager] ⏸️ Tab hidden - pausing quality checks');
        this.pauseQualityMonitoring();
      } else {
        logger.info('[ConnectionManager] ▶️ Tab visible - resuming quality checks');
  ...
  checkConnectionState(peerConnection) {
    ...
    if (iceState === 'connected' && connectionState === 'connected') {
      ...
      if (!this.isReconnecting && (this.connectionState === 'connected' || this.connectionState === 'connecting')) {
        logger.info('[ConnectionManager] ⚠️ Connection lost, attempting reconnection...');
        this.attemptReconnect();
      }
      ...
      if (!this.isReconnecting) {
        logger.info('[ConnectionManager] ❌ Connection failed, attempting reconnection...');
        this.attemptReconnect();
      }
    } else if (iceState === 'checking' || connectionState === 'connecting') {
  ...
  async getConnectionStats(peerConnection, localStream, remoteStream) {
    ...
    try {
      ...
      logger.error('[ConnectionManager] Failed to get stats:', error);
    }
  }
  ...
  attemptReconnect() {
    ...
    if (!this.networkOnline) {
      logger.info('[ConnectionManager] ⏸️ Network offline, waiting for network...');
    ...
    logger.info(`[ConnectionManager] 🔄 Reconnection attempt ${this.reconnectAttempts}/${this.maxReconnectAttempts} in ${delay}ms`);
