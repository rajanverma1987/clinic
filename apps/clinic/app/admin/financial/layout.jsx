...
export default function AdminFinancialLayout({ children }) {
  ...
  return (
    ...
    >
      <div className='admin-page-content'>
        ...
        >
          {ADMIN_FINANCIAL_CHILDREN.map(({ path, labelKey }, index) => {
            const normalizedPath = path.replace(/\/$/, '') || '/';
            const normalizedPathname = (pathname || '').replace(/\/$/, '') || '/';
            const isIndex = normalizedPathname === '/admin/financial';
            const isActive =
              normalizedPathname === normalizedPath ||
              (isIndex && index === 0) ||
              (normalizedPathname.startsWith(normalizedPath + '/') &&
                normalizedPath !== '/admin/financial');
            return (
              <Link
                key={path}
                href={path}
        ...
        <div
          id='admin-financial-panel'
          role='tabpanel'
          aria-labelledby={(() => {
            const i = ADMIN_FINANCIAL_CHILDREN.findIndex(
              (c) => (pathname || '').replace(/\/$/, '') === (c.path || '').replace(/\/$/, ''),
            );
            return i >= 0 ? `admin-financial-tab-${i}` : undefined;
          })()}
        >
          {children}
        </div>
