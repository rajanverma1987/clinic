...
async function postHandler(req, user, { params }) {
  ...
  const doctor = await addDoctorLeave(doctorId, validationResult.data, user.tenantId, user.userId);
...
async function deleteHandler(req, user, { params }) {
  ...
  if (isNaN(leaveIndex) || leaveIndex < 0) {
    return NextResponse.json(errorResponse('Invalid leave index', 'VALIDATION_ERROR'), {
      status: 400,
    });
  }

  const doctor = await removeDoctorLeave(doctorId, leaveIndex, user.tenantId, user.userId);
