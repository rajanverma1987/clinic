...
import { departmentReportSchema } from '@/lib/validations/report';
...
async function getHandler(req, user) {
  ...
  const validationResult = departmentReportSchema.safeParse(queryParams);
  ...
  const reportData = await getDepartmentReport(validationResult.data, user.tenantId, user.userId);

  // Handle CSV export
  if (validationResult.data.format === 'csv') {
    const csv = reportToCSV(reportData, 'Department Report');
