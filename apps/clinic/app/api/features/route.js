...
/** Free trial duration in days per plan. Free Trial / Basic = 6 months; Smart Clinic & Enterprise = 3 months. */
const PLAN_FREE_TRIAL_DAYS = {
  'Free Trial': 180,
  Basic: 180,
  SOLO: 180,
  Core: 180,
  Starter: 180, // 6 months
  'Smart Clinic': 90,
  CLINIC: 90,
  Pro: 90,
  Growth: 90, // 3 months
...
const DEFAULT_FREE_TRIAL_DAYS = 180; // no subscription → 6 months from tenant creation
...
async function getHandler(req, user) {
  ...
  const payload = await optimizedCacheManager.getOrFetch(
    ...
    async () => {
      ...
      // Add-ons only work with a paid plan (not Free Trial / $0)
      const addons =
        hasPaidActiveSub && Array.isArray(sub?.addons)
          ? sub.addons.map((a) => ({ addonKey: a.addonKey }))
          : [];
      return {
        features,
        limits,
