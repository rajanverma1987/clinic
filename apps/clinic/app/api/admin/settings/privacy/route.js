/**
 * GET/PUT /api/admin/settings/privacy
 * Privacy policy content — Super Admin only
 */
...
async function getHandler(req, user) {
  ...
  const content = doc?.content?.privacy || '';
...
async function putHandler(req, user) {
  ...
  await SystemSettings.findOneAndUpdate(
    { slug: SLUG },
    { $set: { 'content.privacy': content, updatedAt: new Date() } },
    { upsert: true },
  );
  return NextResponse.json(successResponse({ content }));
}
