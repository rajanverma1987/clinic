/**
 * Admin Static Page by ID (Super Admin only)
 * GET /api/admin/pages/:id, PUT /api/admin/pages/:id, DELETE /api/admin/pages/:id
 */
...
async function getHandler(req, user, id) {
  ...
  await connectDB();
  const s = await ContentPage.findById(id).lean();
...
async function putHandler(req, user, id) {
  ...
  await connectDB();
  const update = {};
  if (body.key && VALID_KEYS.includes(String(body.key).toLowerCase()))
    update.key = String(body.key).toLowerCase();
  ...
  const s = await ContentPage.findByIdAndUpdate(id, { $set: update }, { new: true }).lean();
  if (!s) return NextResponse.json(errorResponse('Not found', 'NOT_FOUND'), { status: 404 });
  return NextResponse.json(successResponse(toDto(s)));
}
...
async function deleteHandler(req, user, id) {
  ...
  await connectDB();
  const s = await ContentPage.findByIdAndDelete(id);
  if (!s) return NextResponse.json(errorResponse('Not found', 'NOT_FOUND'), { status: 404 });
