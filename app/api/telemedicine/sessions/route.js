...
async function getHandler(req, user) {
  try {
    ...
    return NextResponse.json(
      errorResponse((error instanceof Error ? error.message : String(error)) || 'Failed to fetch sessions', 'FETCH_ERROR'),
      { status: 400 }
    );
  }
}
...
async function postHandler(req, user) {
  try {
    ...
    return NextResponse.json(
      errorResponse((error instanceof Error ? error.message : String(error)) || 'Failed to create session', 'CREATE_ERROR'),
      { status: 400 }
    );
  }
}
