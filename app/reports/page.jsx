  ...
  const renderBarChart = (data, maxValue, height = 200) => {
    ...
    return (
      <div className="relative">
        {/* Y-axis labels */}
        <div className="flex items-end h-[220px] border-b border-l border-gray-300 pl-8 pr-4 pb-8">
          {/* Y-axis scale */}
        ...
        <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
          {entries.map(([label, value], index) => {
            ...
            return (
              ...
              >
                <title>{label}: {typeof value === 'number' ? formatCurrency(value) : value} ({percentage.toFixed(1)}%)</title>
              </path>
            );
          })}
        </svg>
            ...
            return (
              <div key={label} className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium">
                  {typeof value === 'number' ? formatCurrency(value) : value} ({percentage.toFixed(1)}%)
                </span>
              </div>
            );
          })}
  ...
  return (
    <Layout>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Card>
              <div className="p-4">
                ...
                <p className="text-xs text-gray-500 mt-1">{revenueReport.summary.invoiceCount || 0} {t('reports.invoices')}</p>
              </div>
            </Card>
            <Card>
              <div className="p-4">
                <p className="text-sm text-gray-600 mb-1">{t('reports.totalPaid')}</p>
                <p className="text-3xl font-bold text-green-600">
                  {formatCurrency(revenueReport.summary.totalPaid)}
                </p>
                <p className="text-xs text-gray-500 mt-1">{revenueReport.summary.paymentCount || 0} {t('reports.payments')}</p>
              </div>
            </Card>
            <Card>
              <div className="p-4">
                <p className="text-sm text-gray-600 mb-1">{t('reports.totalPending')}</p>
                <p className="text-3xl font-bold text-yellow-600">
                  {formatCurrency(revenueReport.summary.totalPending)}
                </p>
                <p className="text-xs text-gray-500 mt-1">{t('reports.outstanding')}</p>
              </div>
            </Card>
            <Card>
              <div className="p-4">
                <p className="text-sm text-gray-600 mb-1">{t('reports.averageInvoice')}</p>
                ...
                <p className="text-xs text-gray-500 mt-1">{t('reports.perInvoice')}</p>
              </div>
            </Card>
          </div>
          ...
          {revenueReport.timeSeries && revenueReport.timeSeries.length > 0 && (
            <Card>
              ...
              {renderBarChart(revenueReport.timeSeries.map(item => ({ period: item.period, value: item.total })))}
            </Card>
          )}
      ...
      {activeTab === 'patients' && patientReport && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card>
              <div className="p-4">
                <p className="text-sm text-gray-600 mb-1">{t('reports.totalPatients')}</p>
                <p className="text-3xl font-bold text-blue-600">
                  {patientReport.summary.totalPatients || 0}
                </p>
                <p className="text-xs text-gray-500 mt-1">{t('reports.allTime')}</p>
              </div>
            </Card>
            <Card>
              <div className="p-4">
                <p className="text-sm text-gray-600 mb-1">{t('reports.newPatients')}</p>
                <p className="text-3xl font-bold text-green-600">
                  {patientReport.summary.newPatients || 0}
                </p>
                <p className="text-xs text-gray-500 mt-1">{t('reports.inSelectedPeriod')}</p>
              </div>
            </Card>
            <Card>
              <div className="p-4">
                <p className="text-sm text-gray-600 mb-1">{t('reports.activePatients')}</p>
                <p className="text-3xl font-bold text-purple-600">
                  {patientReport.summary.activePatients || 0}
                </p>
                <p className="text-xs text-gray-500 mt-1">{t('reports.withAppointments')}</p>
              </div>
            </Card>
          </div>
          ...
          {patientReport.monthlyTrend && patientReport.monthlyTrend.length > 0 && (
            <Card>
              ...
              {renderBarChart(patientReport.monthlyTrend.map(item => ({ period: item.period, value: item.count })))}
            </Card>
          )}
          ...
          {patientReport.breakdown && (
            <Card>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {patientReport.breakdown.bloodGroups && Object.keys(patientReport.breakdown.bloodGroups).length > 0 && (
                  <div>
                    <h4 className="font-medium mb-2">{t('reports.bloodGroups')}</h4>
                    <div className="space-y-2">
                      {Object.entries(patientReport.breakdown.bloodGroups).map(([group, count]) => (
      ...
      {activeTab === 'appointments' && appointmentReport && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Card>
              <div className="p-4">
                <p className="text-sm text-gray-600 mb-1">{t('reports.totalAppointments')}</p>
                <p className="text-3xl font-bold text-blue-600">
                  {appointmentReport.summary.totalAppointments || 0}
                </p>
                <p className="text-xs text-gray-500 mt-1">{t('common.total')}</p>
              </div>
            </Card>
            <Card>
              <div className="p-4">
                <p className="text-sm text-gray-600 mb-1">{t('reports.completed')}</p>
                <p className="text-3xl font-bold text-green-600">
                  {appointmentReport.summary.completed || 0}
                </p>
                <p className="text-xs text-gray-500 mt-1">
                  {appointmentReport.summary.totalAppointments > 0
                    ? Math.round((appointmentReport.summary.completed / appointmentReport.summary.totalAppointments) * 100)
                    : 0}% {t('reports.completionRate')}
                </p>
              </div>
            </Card>
            <Card>
              <div className="p-4">
                <p className="text-sm text-gray-600 mb-1">{t('appointments.scheduled')}</p>
                <p className="text-3xl font-bold text-yellow-600">
                  {appointmentReport.summary.scheduled || 0}
                </p>
                <p className="text-xs text-gray-500 mt-1">{t('reports.upcoming')}</p>
              </div>
            </Card>
            <Card>
              <div className="p-4">
                <p className="text-sm text-gray-600 mb-1">{t('reports.noShows')}</p>
                <p className="text-3xl font-bold text-red-600">
                  {appointmentReport.summary.noShows || 0}
                </p>
                <p className="text-xs text-gray-500 mt-1">
                  {appointmentReport.summary.totalAppointments > 0
                    ? Math.round((appointmentReport.summary.noShows / appointmentReport.summary.totalAppointments) * 100)
                    : 0}% {t('reports.noShowRate')}
                </p>
              </div>
            </Card>
          </div>
          ...
          {appointmentReport.timeSeries && appointmentReport.timeSeries.length > 0 && (
            <Card>
              ...
              {renderBarChart(appointmentReport.timeSeries.map(item => ({ period: item.period, value: item.count })))}
            </Card>
          )}
      ...
      {activeTab === 'inventory' && inventoryReport && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Card>
              <div className="p-4">
                <p className="text-sm text-gray-600 mb-1">{t('reports.totalItems')}</p>
                <p className="text-3xl font-bold text-blue-600">
                  {inventoryReport.summary.totalItems || 0}
                </p>
                <p className="text-xs text-gray-500 mt-1">{t('reports.activeItems')}</p>
              </div>
            </Card>
            <Card>
              <div className="p-4">
                <p className="text-sm text-gray-600 mb-1">{t('reports.lowStockItems')}</p>
                <p className="text-3xl font-bold text-yellow-600">
                  {inventoryReport.summary.lowStockCount || 0}
                </p>
                <p className="text-xs text-gray-500 mt-1">{t('reports.needReorder')}</p>
              </div>
            </Card>
            <Card>
              <div className="p-4">
                <p className="text-sm text-gray-600 mb-1">{t('reports.expiredItems')}</p>
                <p className="text-3xl font-bold text-red-600">
                  {inventoryReport.summary.expiredCount || 0}
                </p>
                <p className="text-xs text-gray-500 mt-1">{t('reports.expiredBatches')}</p>
              </div>
            </Card>
            <Card>
              <div className="p-4">
                <p className="text-sm text-gray-600 mb-1">{t('reports.totalValue')}</p>
                <p className="text-3xl font-bold text-green-600">
                  {formatCurrency(inventoryReport.summary.totalValue || 0)}
                </p>
                <p className="text-xs text-gray-500 mt-1">{t('reports.inventoryValue')}</p>
              </div>
            </Card>
          </div>
            ...
            <Card>
              ...
              <Table
                columns={[
                  ...
                  { header: t('inventory.lowStockThreshold'), accessor: (row) => row.threshold || 0 },
                  {
                    header: t('common.status'),
                    accessor: (row) => {
                      const stock = row.currentStock || 0;
                      const threshold = row.threshold || 0;
                      if (stock === 0) return <span className="text-red-600 font-medium">{t('inventory.outOfStock')}</span>;
                      if (stock <= threshold) return <span className="text-yellow-600 font-medium">{t('inventory.lowStock')}</span>;
                      return <span className="text-green-600 font-medium">{t('inventory.adequate')}</span>;
                    },
                  },
                ]}
      ...
      {loading && (
          ...
          <div className="text-gray-500">{t('reports.loadingReportData')}</div>
        </div>
      )}

      {!loading && activeTab === 'revenue' && !revenueReport && (
        <Card>
          <p className="text-gray-600 text-center py-8">{t('reports.noRevenueData')}</p>
        </Card>
      )}

      {!loading && activeTab === 'patients' && !patientReport && (
        <Card>
          <p className="text-gray-600 text-center py-8">{t('reports.noPatientData')}</p>
        </Card>
      )}

      {!loading && activeTab === 'appointments' && !appointmentReport && (
        <Card>
          <p className="text-gray-600 text-center py-8">{t('reports.noAppointmentData')}</p>
        </Card>
      )}

      {!loading && activeTab === 'inventory' && !inventoryReport && (
        <Card>
          <p className="text-gray-600 text-center py-8">{t('reports.noInventoryData')}</p>
        </Card>
      )}
    </Layout>
  );
