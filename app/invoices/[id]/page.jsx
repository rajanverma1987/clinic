                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {items.map((item, index) => (
                      <tr key={index}>
                        ...
                        <td className="px-4 py-3 text-sm">{(item.type || '').charAt(0).toUpperCase() + (item.type || '').slice(1)}</td>
                        <td className="px-4 py-3 text-sm">{item.description || ''}</td>
                        <td className="px-4 py-3 text-sm text-center">{item.quantity || 1}</td>
                        <td className="px-4 py-3 text-sm text-right">{formatCurrency(item.unitPrice || 0)}</td>
                        <td className="px-4 py-3 text-sm text-right">{formatCurrency(item.discountAmount || 0)}</td>
                        <td className="px-4 py-3 text-sm text-right">{formatCurrency(item.taxAmount || 0)}</td>
                        <td className="px-4 py-3 text-sm text-right font-medium">{formatCurrency(item.totalWithTax || item.total || 0)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="space-y-3">
                ...
                {(invoice.totalDiscount || 0) > 0 && (
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">-{formatCurrency(invoice.totalDiscount || 0)}</span>
                  </div>
                )}
                ...
                {(invoice.balanceAmount || 0) > 0 && invoice.status !== 'paid' && (
                  <div className="flex justify-between text-lg font-bold border-t pt-3">
                    ...
                    <span className="text-red-600">{formatCurrency(invoice.balanceAmount || 0)}</span>
                  </div>
                )}
                {invoice.status === 'paid' && (
                  <div className="flex justify-between text-lg font-bold border-t pt-3">
