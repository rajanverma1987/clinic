...
interface TenantSettings {
  ...
  settings: {
    ...
    queueSettings?: {
      displayOrder: 'priority' | 'fifo' | 'appointment_time';
...
type Tab = 'general' | 'compliance' | 'doctors' | 'hours' | 'queue' | 'tax' | 'profile';
...
export default function SettingsPage() {
  ...
  const [queueForm, setQueueForm] = useState({
    displayOrder: 'priority' as 'priority' | 'fifo' | 'appointment_time',
  ...
  const [taxForm, setTaxForm] = useState({
    country: '',
    taxType: 'SALES_TAX' as 'GST' | 'VAT' | 'SALES_TAX',
    rate: 0,
  });
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        if (data.settings.clinicHours && Array.isArray(data.settings.clinicHours) && data.settings.clinicHours.length > 0) {
          const hours = data.settings.clinicHours.map((hour: any) => {
          ...
          const loadedDays = hours.map((h: any) => h.day);
  ...
  const updateClinicHour = (index: number, field: keyof ClinicHours, value: any) => {
  ...
  // Define all tabs
  const allTabs = [
    { id: 'profile' as Tab, label: t('settings.profile'), icon: <ProfileIcon />, adminOnly: false },
    { id: 'general' as Tab, label: t('settings.clinicInfo'), icon: <ClinicIcon />, adminOnly: true },
    { id: 'compliance' as Tab, label: t('settings.compliance'), icon: <ComplianceIcon />, adminOnly: true },
    { id: 'doctors' as Tab, label: t('settings.doctorsStaff'), icon: <DoctorsIcon />, adminOnly: true },
    { id: 'hours' as Tab, label: t('settings.clinicHours'), icon: <HoursIcon />, adminOnly: false },
    { id: 'queue' as Tab, label: t('settings.queueSettings'), icon: <QueueIcon />, adminOnly: false },
    { id: 'tax' as Tab, label: t('settings.taxSettings'), icon: <TaxIcon />, adminOnly: false },
  ];
          ...
          <div className="space-y-6">
            <div>
              ...
              <select
                ...
                onChange={(e) => setQueueForm({ ...queueForm, displayOrder: e.target.value as any })}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
