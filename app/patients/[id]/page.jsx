    ...
    try {
      ...
      try {
        ...
        if (aptResponse.success && aptResponse.data) {
          const aptData = Array.isArray(aptResponse.data) ? aptResponse.data : aptResponse.data.data || [];
      ...
      try {
        ...
        if (presResponse.success && presResponse.data) {
          const presData = Array.isArray(presResponse.data) ? presResponse.data : presResponse.data.data || [];
      ...
      try {
        ...
        if (invResponse.success && invResponse.data) {
          const invData = Array.isArray(invResponse.data) ? invResponse.data : invResponse.data.data || [];
          setInvoices(invData);
        }
      } catch (err) {
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  {isEditing ? (
                    ...
                    <p className="text-gray-900">{patient.alternatePhone || 'Not Provided'}</p>
                  )}
                </div>
                <div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  {isEditing ? (
                    ...
                    <p className="text-gray-900 whitespace-pre-wrap">{patient.medicalHistory || 'Not Provided'}</p>
                  )}
                </div>
                <div>
                ...
                <div>
                  ...
                  {isEditing ? (
                    ...
                    <p className="text-gray-900 whitespace-pre-wrap">{patient.currentMedications || 'None'}</p>
                  )}
                </div>
                <div>
                  ...
                  {isEditing ? (
                    ...
                    <p className="text-gray-900 whitespace-pre-wrap">{patient.notes || 'No Notes'}</p>
                  )}
                </div>
              </div>
            </Card>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {appointments.map((apt) => (
                    <tr key={apt._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm">{apt.startTime || '-'}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm capitalize">{apt.type || 'In-Person'}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        {apt.doctorId ? `Dr. ${apt.doctorId.firstName} ${apt.doctorId.lastName}` : '-'}
                      </td>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {prescriptions.map((pres) => (
                    <tr key={pres._id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">{pres.prescriptionNumber}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 py-1 text-xs rounded-full ${
                          ...
                          pres.status === 'dispensed' ? 'bg-blue-100 text-blue-800' :
                          'bg-gray-100 text-gray-800'
                        }`}>
                          {pres.status}
                        </span>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <div className="flex gap-2">
                          ...
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => window.open(`/prescriptions/${pres._id}/print`, '_blank')}
                          >
                            Print
                          </Button>
                        </div>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {invoices.map((inv) => (
                    <tr key={inv._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 text-sm">{inv.items.length} item{inv.items.length !== 1 ? 's' : ''}</td>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {labTests.map((test, index) => (
                    <tr key={test._id || index} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">{test.testName}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm">{test.testCode || '-'}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
