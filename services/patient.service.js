...
async function generatePatientId(tenantId) {
  ...
  const patientId = lastPatient.patientId;
  if (!patientId) {
    return 'PAT-0001';
  }
 ...
 * Get patient by ID
 */
export async function getPatientById(patientId, tenantId, userId) {
  await connectDB();
 ...
 */
export async function listPatients(query, tenantId, userId) {
  await connectDB();
  ...
  return createPaginationResult(patients, total, page || 1, limit || 10);
}

/**
 * Update patient
 */
export async function updatePatient(patientId, input, tenantId, userId) {
  await connectDB();
 ...
 * Soft delete patient
 */
export async function deletePatient(patientId, tenantId, userId) {
  await connectDB();
